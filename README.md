...
dr_analysis() – fits both 3‑parameter and 4‑parameter log‑logistic models, compares them using AIC/BIC, returns ED 10, 50, and 90 values, parameter comparisons between populations, and a dose-response curve.<br>
...
### Data Requirements
The dr_analysis() function uses functions from the drc and readxl packages, so make sure they are downloaded before running the dr_analysis() function. <br> If the following data requirements are not met, the function will return an error. Be aware, these are case-sensitive.<br>
...
An example of a properly formatted input file is provided in the repository ("dr_data_normalized_example.xslx").
...
### Data requirements
The normalize_dr_biomass() function uses functions from the readxl package, so make sure it is downloaded before running the normalize_dr_biomass() function. <br>
Your input must be an Excel file (.xlsx) with the following columns:<br>
* population (character): Identifies population/biotype. Example: sensitive, resistant<br>
...
If your data does not include an untreated control, it should. But if it doesn't, treated rows in that group will get NA for bio.pctrl (no valid denominator).<br>
An example of a properly formatted input file is provided in the repository ("dr_data_example.xlsx").
